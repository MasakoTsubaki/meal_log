/* ====================================================
   食事＆活動ログ v7 完全版（再読込対応・抽出重複防止）
   ==================================================== */

function todayISO() { return new Date().toISOString().slice(0,10); }

const LS_LOGS='meal_logs_v7', LS_COMMENTS='meal_comments_v7', LS_GOALS='meal_goals_v7', LS_EXTRACTED='meal_extracted_v7';
...
let extractedFoodsByDate = JSON.parse(localStorage.getItem(LS_EXTRACTED)||'{}');
...
function saveExtracted(){ localStorage.setItem(LS_EXTRACTED, JSON.stringify(extractedFoodsByDate)); }
...
function normalizeFoodName(name) {
  ...
  const map = {
    'ミニトマト': 'トマト', 'マグロ': 'まぐろ', 'タラ': 'たら', '鱈': 'たら',
    'タケノコ': 'たけのこ', 'ナス': 'なす', 'しいたけ': '椎茸', 'まいたけ': '舞茸',
    'エリンギ': 'エリンギ', '卵': '卵', 'タマゴ': '卵', 'たまご': '卵', '玉子': '卵', 'エッグ': '卵',
    '目玉焼き': '目玉焼き'
  };
  for (const [key, value] of Object.entries(map)) {
    if(n.includes(key)) n = n.replace(new RegExp(key, 'g'), value);
  }
  return n;
}
...
/* ====================
   重複カウント（抽出食材ベース）
   ==================== */
function countExtractedFoodsPerDay(extractedFoodsArray, date){
  const counts = {};
...
function renderDateLogs(date){
  ...
  dayLogs.forEach(entry=>{
    ...
    del.onclick=()=>{ if(!confirm('削除しますか？')) return; logs=logs.filter(x=>x.id!==entry.id); saveLogs(); renderDateLogs(date); };
...
function updateSummaryFromExtracted(date){
  const extracted = extractedFoodsByDate[date]||[];
  const counts = countExtractedFoodsPerDay(extracted, date);
...
document.getElementById('addFoodBtn').addEventListener('click', ()=>{
  ...
  addUserFoodInput(date,userInput);
...
/* ====================
   ...
   ==================== */
async function init(){
  await loadCategoryCSV();

  // 過去ログを日付ごとに抽出済み食材に再構築
  const dates = [...new Set(logs.filter(l=>l.type==='food').map(l=>l.date))];
  dates.forEach(date=>{
    extractedFoodsByDate[date] = [];
    logs.filter(l=>l.date===date && l.type==='food').forEach(l=>addUserFoodInput(date, l.name));
  });
